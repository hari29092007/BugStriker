...
Converted from `bugstriker.zip`. This document preserves the archive text files as source reference; compiled Python cache files are intentionally omitted.
...
``````json
{
  ...
  "metadata": {
    "app": "BugStriker",
    "version": "1.0.0",
    "last_sync": "2026-09-19T05:33:37.702Z"
  }
}
``````
...
def main():
    ...
    if args.json_input:
        ...
        # Load buggy starter code as default demo
        prob = load_problem_rubric(problem_id)
        code = prob.get("buggy_starter_code", "")
...
MODEL_NAME = "gemini-2.5-flash"  # standard v1beta REST endpoint model
...
def call_gemini_rest(prompt: str, system_instruction: str = "", response_schema: Optional[Dict[str, Any]] = None) -> str:
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            if not parts:
                raise ValueError("Empty parts returned from Gemini API")
...
STRICT NEGATIVE CONSTRAINT: DO NOT provide code, syntax fixes, or replacement implementation.`,
        ...
        console.error("Gemini probe generation error:", err);
      }
    }
...
STRICT NEGATIVE CONSTRAINT: DO NOT provide the correct code or replacement implementation in your feedback.`,
      ...
      console.error("Gemini explanation grading error:", err);
    }
  }
...
STRICT NEGATIVE CONSTRAINT: DO NOT provide code solutions.`,
      ...
      if (response.text) {
        verdictObj = JSON.parse(response.text);
      }
    } catch (err) {
      console.error("Gemini verdict generation error:", err);
    }
  }
...
export default function App() {
    ...
    async function init() {
      try {
        ...
        if (rubricsData.problems && rubricsData.problems.length > 0) {
          ...
          setCode(initialProblem.buggy_starter_code);
  ...
  const handleSelectProblem = async (problemId: string) => {
    ...
    setCode(found.buggy_starter_code);
    setErrorMsg(null);
  ...
  const handleResetSession = async () => {
    ...
    setErrorMsg(null);
    setCode(currentProblem.buggy_starter_code);
    try {
      setLoading(true);
  ...
  const handleResumeSession = async (sessionId: string) => {
    ...
    try {
      ...
      if (data.submission_v2?.code) {
        ...
        setCode(prob.buggy_starter_code);
      }
    } catch (err: any) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-7 space-y-4">
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden shadow-sm flex flex-col">
              ...
              {/* Editor Top Bar */}
              <div className="flex items-center justify-between px-3 py-2 bg-neutral-950 border-b border-neutral-800 text-xs font-mono">
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      if (currentProblem) setCode(currentProblem.buggy_starter_code);
                    }}
                    disabled={isTerminal || loading}
                    className="text-[11px] text-neutral-400 hover:text-neutral-200 transition-colors disabled:opacity-50"
                  >
                    Reset Code
                  </button>
                </div>
              ...
              <div className="relative p-2 bg-neutral-950 font-['Fira_Code',monospace] text-xs">
                <textarea
                  ...
                  className="w-full bg-neutral-950 text-amber-200/90 font-['Fira_Code',monospace] text-xs p-2 leading-relaxed resize-y focus:outline-none border border-neutral-800/80 rounded-md focus:border-amber-500/80 disabled:opacity-75"
                />
              </div>
